...
with open('diffusion/alzheimers_class_index.json', 'r') as f:
    ALZHEIMERS_CLASS_INDEX = json.load(f)
FOLDER_TO_CLASS_AD = {folder: int(i) for i, (folder, _) in ALZHEIMERS_CLASS_INDEX.items()}
...
class Alzheimers(torch.utils.data.Dataset):
    def __init__(
        ...
        super(Alzheimers, self).__init__()
        self.root = os.path.join(root, 'alzheimers-32')
        self._images = glob.glob(self.root + '/*/*')
        self._images.sort()
        self.transform = transform
        assert len(self._images) == 16, f"Expected {len(self)} images, found {len(self._images)}"
    ...
    def path_to_cls(self, path):
        folder = path.split('/')[-2]
        return FOLDER_TO_CLASS_AD[folder]
...
def get_target_dataset(name: str, train=False, transform=None, target_transform=None):
    ...
    #     dataset.file_to_class = None
    #     dataset.class_to_idx = None
    elif name == 'alzheimers-32':
        dataset = Alzheimers(root=DATASET_ROOT, transform=transform)
        dataset.file_to_class = None
        dataset.class_to_idx = None
